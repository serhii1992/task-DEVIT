...
const multiplyArr = (arr) => {
  const minNumInArr = Math.min(
    ...arr.reduce((acc, element) => acc.concat(element),[])
  )
...
const arrToObject = (arr) => {
  ...
  for (let el of arr) {
    ...
    if(!Array.isArray(value)){
     ...
     result[key] = arrToObject(value)
    }
  }
  return result
};
console.log(arrToObject(arr5));
